...
| `/ipa-sec-start` | 通報対象の整理を始める |
| `/ipa-sec-cwe` | CWE候補を整理する |
| `/ipa-sec-severity` | CVSSと深刻度・影響範囲の下書きを作る |
| `/ipa-sec-draft` | IPAフォームに転記できる下書きを作る |
| `/ipa-sec-checklist` | 添付漏れ・記入漏れを確認する |
| `/ipa-sec-export` | 提出用の Markdown と JSON を保存する |
...
- まだ何も整理していない → `/ipa-sec-start`
- CWEで止まっている → `/ipa-sec-cwe`
- 深刻度で止まっている → `/ipa-sec-severity`
- 本文で止まっている → `/ipa-sec-draft`
- 提出前で不安 → `/ipa-sec-checklist`
- 保存したい → `/ipa-sec-export`
